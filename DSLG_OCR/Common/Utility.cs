...
namespace DSLG_OCR.Common
{
    class Utility
    {
        ...
        ///     結果表示文字列</param>
        /// ------------------------------------------------------------------
        public static void logOutput(string sb, string t)
        {
            // ログ出力先
            if (!System.IO.Directory.Exists(Properties.Settings.Default.logPath))
            {
                // フォルダが存在しないときは作成する
                System.IO.Directory.CreateDirectory(Properties.Settings.Default.logPath);
            }
            ...
            // ストリングリーダーのインスタンスを生成
            System.IO.StringReader sr = new System.IO.StringReader(sb);

            // 1行ずつ読みこむ
            string st = string.Empty;
            while ((st = sr.ReadLine()) != null)
            {
                Array.Resize(ref pData, iX + 1);
                pData[iX] = st;
                iX++;
            }
            ...
            if (pData != null)
            {
                ...
                // ファイル名
                string outFileName = Properties.Settings.Default.logPath + t + "_" + newFileName + ".csv";

                // テキストファイル出力
                System.IO.File.WriteAllLines(outFileName, pData, System.Text.Encoding.GetEncoding(932));
            }
